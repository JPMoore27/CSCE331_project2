...
public class GUI extends JFrame implements ActionListener {
    ...
    private void showItemsOrderedByItemID() {
        ...
        //Making Excess Report Button
        JButton excessReportButton = new JButton("Excess Report");
        excessReportButton.setBackground(new Color(0x0096FF));
        excessReportButton.setPreferredSize(new Dimension(BUTTON_WIDTH, BUTTON_HEIGHT));
        excessReportButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String timestampInput = JOptionPane.showInputDialog("Enter a timestamp (e.g., YYYY-MM-DD HH:MM:SS):");

                if (timestampInput != null) {
                    Timestamp timestamp = Timestamp.valueOf(timestampInput);
                    List<String> excessItems = calculateExcessReport(timestamp);
                    displayExcessReportDialog(excessItems);
                }
            }
        });
    ...
    // Create a method to calculate the Excess Report based on the provided timestamp
    private List<String> calculateExcessReport(Timestamp timestamp) {
        List<String> excessItems = new ArrayList<>();

        try {
            // Query the database to calculate items with sales less than 10% of their initial inventory
            String query = "SELECT i.itemid, i.quantity, SUM(s.quantity) AS TotalSold "
                    + "FROM items i "
                    + "LEFT JOIN orders s ON i.itemid = s.itemid AND s.time >= ? "
                    + "GROUP BY i.itemid, i.quantity "
                    + "HAVING COALESCE(TotalSold, 0) < 0.1 * i.quantity";
            ...
            while (resultSet.next()) {
                ...
                int totalOrdered = resultSet.getInt("TotalOrdered");

                excessItems.add("Item ID: " + itemId + " (Initial: " + quantity + ", Ordered: " + totalOrdered + ")");
            }

            preparedStatement.close();
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error calculating excess report", e);
            JOptionPane.showMessageDialog(null, "Error calculating excess report.");
        }
    ...
    // Create a method to display the Excess Report in a dialog
    private void displayExcessReportDialog(List<String> excessItems) {
        if (excessItems.isEmpty()) {
            JOptionPane.showMessageDialog(null, "No items with excess inventory.");
        } else {
            StringBuilder reportMessage = new StringBuilder("Excess Inventory Report:\n");
            for (String item : excessItems) {
                reportMessage.append(item).append("\n");
            }
            JOptionPane.showMessageDialog(null, reportMessage.toString());
        }
    }
